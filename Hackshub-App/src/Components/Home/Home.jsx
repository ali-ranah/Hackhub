...
//             <Card className="w-[90vw] items-start justify-center rounded-2xl hover:shadow-black hover:shadow-2xl transition-transform  hover:-translate-y-1">
//               <Card.Content className="flex flex-col items-start">
//               <Text className="text-sm font-bold self-center text-black mb-[2vh]">{event.event_title}</Text>
...
//                   <Text className="text-sm font-bold text-black">
//                   Time Allowed: {event.allowed_time} {parseInt(event.allowed_time) === 1 ? 'hour' : 'hours'}                  </Text>
//                 </View>
//                 <View className="flex-row">
...
const ReadMoreText = ({ text, numberOfLines, textLength }) => {
          ...
          {events.map((event) => (
            <View key={event.id} className="w-full flex mb-2 items-center justify-center">
              <Card className='w-full items-start justify-center rounded-2xl hover:shadow-black hover:shadow-2xl transition-transform hover:-translate-y-1'>
                <Card.Content className="flex flex-col items-start">
                <Text className="text-sm font-bold text-black">{event.event_title}</Text>
                  <View className='flex-row'>
                    <MaterialIcons name="date-range" size={20} color="black" />
                     <Text className="text-sm font-bold text-black">
                      {formatDate(event.start_date)} to {formatDate(event.end_date)}
                  ...
                  <View className='flex-row'>
                    <MaterialIcons name="question-answer" size={20} color="black" />
                    <ReadMoreText text={`Question: ${event.question}`} numberOfLines={2} textLength={50} />
                  </View>
                  <View className='flex-row'>
                    <MaterialIcons name="rule" size={20} color="black" />
                    <ReadMoreText text={`Guidelines: ${event.guidelines}`} numberOfLines={2} textLength={50} />
                  </View>
                  <View className='flex-row'>
                    <MaterialIcons name="description" size={20} color="black" />
                    <ReadMoreText text={`Description: ${event.description}`} numberOfLines={2} textLength={50} />
                  </View>
                  <View className='flex-row'>
                    <MaterialIcons name="attach-money" size={20} color="black" />
